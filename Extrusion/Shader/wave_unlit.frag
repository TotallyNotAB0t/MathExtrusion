...
{   
    ...
    vec3 color = texture(texSampler[ubm[imaterial].albedoMap], ftc).rgb * ubm[imaterial].albedo * Color; 
    vec3 ambient = vec3(0.003) * color * ubm[imaterial].ao * texture(texSampler[ubm[imaterial].aoMap], ftc).rgb;
    vec3 metallic = texture(texSampler[ubm[imaterial].metallicMap], ftc).rgb * ubm[imaterial].metallic;
    float roughness = texture(texSampler[ubm[imaterial].roughnessMap], ftc).r * ubm[imaterial].roughness;
    vec3 normal = texture(texSampler[ubm[imaterial].normalMap], ftc).rgb * 2.0;
